...
\begin{displaymath}
	l_{vehicle} = \frac{\sum\limits_{\substack{(x,y) \in \text{TB} \\ \boldsymbol{TI_{\text{Size}}}(x,y)\neq 0}} \boldsymbol{TI_{\text{Size}}}(x,y)}{n_{\text{NZpoints}}},
\end{displaymath}
...
The sizes on the Size Timeline are converted to metrics with a scaling-factor that is specified in the configuration files, and depends on the specific traffic scene and frame setting.
...
\begin{figure}[!t]
	...
	\caption{Some particular vehicle appearances representing the system's classification categories.\label{fig:types}}
    \end{adjustbox}
\end{figure}
%----------------------------------------------------------------------------
