...
At the fourth and fifth test stage, the minimal split size is decreased to a smaller values than the regular minimal blob size.
As chart \ref{chart:recalls}.~shows, the recalls only slightly changed, and most accuracies (chart \ref{chart:accuracies}.) either increased or stagnated correspondingly.
This is due to the increased number of false positives and the decreased number of false negatives, that compensate each other's effects.


\subsubsection{Outliers}
As the results charts show, some videos do not follow the common tendencies described in section \ref{sec:ver_2}.,\ref{sec:ver_3}.,\ref{sec:ver_4}., but  
